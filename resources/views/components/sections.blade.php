...
@endisset
@isset($populate)
      <section class="populate display-none column p-10 justify-center pos-fixed top-0 left-0 bottom-0 right-0 bg-black-transparent z-index-8000">
        
        <div class="w-full form column max-w-500 bg p-10 br-10 g-10">
           <div class="row m-x-auto c-primary">
...
<path fill-rule="evenodd" clip-rule="evenodd" d="M10.6669 5.13443L10.165 4.77922C9.44862 4.27225 8.59264 4 7.71504 4H7.10257C6.69838 4 6.29009 4.02549 5.90915 4.16059C3.52645 5.00566 1.88749 8.09504 2.00604 15.1026C2.02992 16.5145 2.3603 18.075 3.63423 18.6842C4.03121 18.8741 4.49667 19 5.02671 19C5.66273 19 6.1678 18.8187 6.55763 18.5632C6.96641 18.2953 7.32633 17.9471 7.68612 17.599C8.13071 17.1688 8.57511 16.7389 9.11125 16.4609C9.69519 16.1581 10.3434 16 11.0011 16H12.9989C13.6566 16 14.3048 16.1581 14.8888 16.4609C15.4249 16.7389 15.8693 17.1688 16.3139 17.599C16.6737 17.9471 17.0336 18.2953 17.4424 18.5632C17.8322 18.8187 18.3373 19 18.9733 19C19.5033 19 19.9688 18.8741 20.3658 18.6842C21.6397 18.075 21.9701 16.5145 21.994 15.1026C22.1125 8.09503 20.4735 5.00566 18.0908 4.16059C17.7099 4.02549 17.3016 4 16.8974 4H16.2849C15.4074 4 14.5514 4.27225 13.8351 4.77922L13.3332 5.13441C12.9434 5.41029 12.4776 5.55844 12 5.55844C11.5225 5.55844 11.0567 5.41029 10.6669 5.13443ZM16.75 8C17.1642 8 17.5 8.33579 17.5 8.75C17.5 9.16421 17.1642 9.5 16.75 9.5C16.3358 9.5 16 9.16421 16 8.75C16 8.33579 16.3358 8 16.75 8ZM7.5 8.25C7.91421 8.25 8.25 8.58579 8.25 9V9.75H9C9.41421 9.75 9.75 10.0858 9.75 10.5C9.75 10.9142 9.41421 11.25 9 11.25H8.25V12C8.25 12.4142 7.91421 12.75 7.5 12.75C7.08579 12.75 6.75 12.4142 6.75 12V11.25H6C5.58579 11.25 5.25 10.9142 5.25 10.5C5.25 10.0858 5.58579 9.75 6 9.75H6.75V9C6.75 8.58579 7.08579 8.25 7.5 8.25ZM19 10.25C19 10.6642 18.6642 11 18.25 11C17.8358 11 17.5 10.6642 17.5 10.25C17.5 9.83579 17.8358 9.5 18.25 9.5C18.6642 9.5 19 9.83579 19 10.25ZM15.25 11C15.6642 11 16 10.6642 16 10.25C16 9.83579 15.6642 9.5 15.25 9.5C14.8358 9.5 14.5 9.83579 14.5 10.25C14.5 10.6642 14.8358 11 15.25 11ZM17.5 11.75C17.5 11.3358 17.1642 11 16.75 11C16.3358 11 16 11.3358 16 11.75C16 12.1642 16.3358 12.5 16.75 12.5C17.1642 12.5 17.5 12.1642 17.5 11.75Z" fill="CurrentColor"></path>
            ...
            <span class="c-primary m-x-auto flex text-center" style="font-family:luckiest guy;font-size:2rem;">Fund Games Wallet</span>
            <span>Enter Voucher Code here</span>
            <div class="h-full cont w-full border-1 border-color-silver br-5 h-50 row align-center bg-black-transparent">
                <input type="text" placeholder="E.G ASDFGHJKIUYTRD" class="inp input w-full align-center h-full border-none bg-transparent br-10">
            </div>
              <a href="{{ url('voucher/vendors') }}" class="m-left-auto c-green no-u">Buy Voucher Code</a>
           <div style="border:0.1px dashed #4caf50;background:rgba(0,255,0,0.1)" class="w-full c-green display-none prompt success p-10 br-5">


           </div>
            <div style="border:0.1px dashed red;background:rgba(255,0,0,0.1)" class="w-full c-red display-none prompt error p-10 br-5">


           </div>
           <button onclick="
           try{
           document.querySelectorAll('.prompt').forEach((data)=>{
           data.classList.add('display-none');
           });
            if(this.closest('.form').querySelector('input').value == ''){
            this.closest('.form').querySelector('.cont').classList.add('empty');
            }else{
              this.closest('.form').querySelector('.cont').classList.remove('empty');
            GetRequest(event,'{{ url('users/get/redeem/voucher/process') }}?code=' + this.closest('.form').querySelector('input').value,this,MyFunc.Redeemed);

            }
           }catch(error){
           alert(error.stack)
           }
            " class="btn-primary-3d w-full clip-5 br-5">Add To Wallet</button>
          
        </div>
        <span onclick=" document.querySelector('.populate').classList.add('shown');document.querySelector('.populate').classList.add('display-none');document.body.classList.remove('overflow-hidden')" style="color:silver" class="c-silver close-modal pc-pointer column m-x-auto m-top-50">
            <svg width="30" height="30" viewBox="0 0 24 24" fill="CurrentColor" xmlns="http://www.w3.org/2000/svg">
<path d="M10.0303 8.96965C9.73741 8.67676 9.26253 8.67676 8.96964 8.96965C8.67675 9.26255 8.67675 9.73742 8.96964 10.0303L10.9393 12L8.96966 13.9697C8.67677 14.2625 8.67677 14.7374 8.96966 15.0303C9.26255 15.3232 9.73743 15.3232 10.0303 15.0303L12 13.0607L13.9696 15.0303C14.2625 15.3232 14.7374 15.3232 15.0303 15.0303C15.3232 14.7374 15.3232 14.2625 15.0303 13.9696L13.0606 12L15.0303 10.0303C15.3232 9.73744 15.3232 9.26257 15.0303 8.96968C14.7374 8.67678 14.2625 8.67678 13.9696 8.96968L12 10.9393L10.0303 8.96965Z" fill="CurrentColor"></path>
